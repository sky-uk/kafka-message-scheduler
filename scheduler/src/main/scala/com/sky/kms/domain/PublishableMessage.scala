...
object PublishableMessage {
  ...
  case class ScheduledMessage(inputTopic: String, outputTopic: String, key: Array[Byte], value: Option[Array[Byte]]) extends PublishableMessage
  ...
  implicit val scheduledMessageProducerRecordEnc: ProducerRecordEncoder[ScheduledMessage] =
    ProducerRecordEncoder.instance(message => new ProducerRecord(message.outputTopic, message.key, message.value.orNull))
