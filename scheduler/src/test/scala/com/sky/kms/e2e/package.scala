...
package object e2e {
  ...
  def withSchedulerApp[T](scenario: => T)(implicit conf: SchedulerConfig, system: ActorSystem): T =
    withRunningScheduler(SchedulerApp.configure apply AppConfig(conf))(_ => scenario)

  def withRunningScheduler[T](schedulerApp: SchedulerApp)(scenario: SchedulerApp.Running => T)(
