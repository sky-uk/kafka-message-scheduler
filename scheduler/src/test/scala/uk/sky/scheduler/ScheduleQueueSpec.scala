...
final class ScheduleQueueSpec extends AsyncWordSpec, AsyncIOSpec, Matchers, OptionValues, EitherValues {
  ...
  "ScheduleQueue" when {
    ...
    "updating the Cancelable Fiber Ref" should {
      "add a Cancelable Fiber to the Ref when it is scheduled" in withContext {
      ...
      "remove a Cancelable Fiber from the Ref when it is canceled" in withContext {
      ...
      "remove a Cancelable Fiber from the Ref when it is due" in withContext {
        case TestContext(repository, allowEnqueue, _, scheduleQueue, scheduleEvent) =>
          for {
            _             <- allowEnqueue.complete(())
